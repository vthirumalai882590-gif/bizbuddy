...
router.get('/', async (req, res) => {
    const expenses = await (0, database_1.loadExpenses)(req.uid);
...
router.post('/', async (req, res) => {
    ...
    await (0, database_1.saveExpenses)(expenses, userId);
...
router.delete('/:id', async (req, res) => {
    const userId = req.uid || 'demo-user';
    let expenses = await (0, database_1.loadExpenses)(userId);
...
router.get('/ai-coach', async (req, res) => {
    ...
    try {
        ...
        const incomes = await (0, database_1.loadIncome)(userId);
    ...
    Speak in a clear, friendly, and practical manner.`;
        const adviceText = await (0, gemini_1.generateText)(prompt);
        ...
        const categoryTotals = Object.entries(expenses.reduce((acc, e) => {
            acc[e.category] = (acc[e.category] || 0) + e.amount;
            return acc;
        }, {})).sort((a, b) => b[1] - a[1]);
...
router.post('/chat', async (req, res) => {
    ...
    Provide a short, friendly, and highly actionable response (maximum 2-3 sentences) directly answering their business problem.`;
        const chatHistory = history.length > 0 ? history : [{ role: 'user', content: message }];
        const reply = await (0, gemini_1.generateChatReply)(chatHistory, systemInstruction);
        res.json({
            success: true,
            reply: reply
        });
