...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (user) {
      if (user.uid === 'demo-user') {
        ...
        auth.currentUser?.getIdToken().then(() => setTokenReady(true)).catch(() => setTokenReady(true))
      }
    } else {
      setTokenReady(false)
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {overview.totalIncome === 0 && overview.totalExpenses === 0 && user?.uid !== 'demo-user' && (
        <div className="bg-gradient-to-r from-brand-50 to-brand-100 border border-brand-200 rounded-2xl p-6 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="space-y-1">
            <h3 className="font-bold text-slate-800 text-sm">💡 Ready to explore BizBuddy AI?</h3>
            <p className="text-xs text-gray-500 max-w-2xl">
              Since this is a brand new account, your dashboard is empty. You can populate it with sample expenses, income, website configurations, and receipts to see the AI advisor and forecasts in action.
            </p>
          </div>
          <button
