...
export default function AIAdvisorPage() {
  ...
  useEffect(() => {
    try {
      const storageKey = `bizbuddy_chat_history_${user?.uid || 'demo-user'}`
      const saved = localStorage.getItem(storageKey)
      if (saved) {
        setMessages((JSON.parse(saved) as ChatMessage[]).map(m => ({ ...m, timestamp: new Date(m.timestamp) })))
      } else {
        setMessages([])
      }
    } catch {
  ...
  useEffect(() => {
    if (!user) return
    try {
      const storageKey = `bizbuddy_chat_history_${user.uid}`
