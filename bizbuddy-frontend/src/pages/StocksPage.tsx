...
export default function StocksPage() {
  ...
  useEffect(() => {
    try {
      const storageKey = `bizbuddy_stocks_ledger_${user?.uid || 'demo-user'}`
      const stored = localStorage.getItem(storageKey)
      const parsed = stored ? JSON.parse(stored) : []
      if (Array.isArray(parsed)) {
        setStocks(parsed.map(normalizeStockItem))
      } else {
        setStocks([])
      }
    } catch {
  ...
  useEffect(() => {
    if (!user) return
    try {
      const storageKey = `bizbuddy_stocks_ledger_${user.uid}`
...
Use ₹ for currency. Be direct and practical, like a trusted business advisor, not generic.`
  ...
  const handleDeleteStock = (id: string) => {
    if (!window.confirm('Are you sure you want to delete this stock item from your inventory?')) return
