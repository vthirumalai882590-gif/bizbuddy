...
// Inject Firebase ID token or x-demo-mode header on every request
api.interceptors.request.use(async (config) => {
  const isDemo = localStorage.getItem('bizbuddy_demo_mode') === 'true'
  ...
  if (user && !isDemo) {
    try {
      ...
      console.warn('[API] Error retrieving ID token:', err)
...
api.interceptors.response.use(
  ...
  (err) => {
    ...
    console.warn('[API Error]', err.response?.status, msg)
