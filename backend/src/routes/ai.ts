...
router.get('/business-health', authMiddleware, async (req: AuthRequest, res) => {
  res.json({
    success: true,
    data: {
      score: 72,
...
router.post('/marketing', authMiddleware, async (req: AuthRequest, res) => {
  try {
...
router.post('/website', authMiddleware, async (req: AuthRequest, res) => {
